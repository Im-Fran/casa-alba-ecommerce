<div>
    <x-drawer title="Carrito de Compras" wire:model="openCart" class="w-11/12 lg:w-1/3" right withCloseButton>
        @php($lines = $this->cart?->lines ?: [])
        <div class="flex flex-col items-stretch justify-between w-full h-[80vh] md:h-[90vh]">
            <div class="flex flex-col max-h-[70vh]">
                ...
                @empty
                    <div class="flex flex-col items-center justify-center w-full h-[70vh]">
                        ...
                        <x-button wire:click="$toggle('openCart')" class="btn-primary btn-outline text-neutral-50 mt-4" label="Seguir Comprando"/>
                    </div>
                @endforelse
            </div>
            @if(count($lines) > 0)
    ...
    <x-lucide-shopping-bag wire:click.stop="$toggle('openCart')" class="hover:text-primary w-8 h-8 md:w-6 md:h-6"/>
</div>
